/**
 * Copyright (C) 2024 Moesif (https://moesif.com)
 *
...
public class MoesifPolicy implements Policy {
    ...
    public Completable onRequest(HttpExecutionContext ctx) {
        ...
        if (configuration.userIdHeader != null && request.headers().contains(configuration.userIdHeader)) {
            eventModel.setUserId(request.headers().get(configuration.userIdHeader));
        }
        if (configuration.companyIdHeader != null && request.headers().contains(configuration.companyIdHeader)) {
            eventModel.setCompanyId(request.headers().get(configuration.companyIdHeader));
        }
        ctx.setAttribute("eventModel", eventModel);

        return request.body()
                .doOnSuccess(buffer -> {
                    eventRequestModel.setBody(buffer.toString());
    ...
    public Completable onResponse(HttpExecutionContext ctx) {
        ...
        EventModel eventModel = Optional.ofNullable((EventModel) ctx.getAttribute("eventModel")).orElseThrow();
        ...
        return response.body()
                .doOnSuccess(buffer -> {
                    ...
                    eventResponseModel.setBody(buffer.toString());
                })
                .doOnError(error -> {
    ...
    private void logMoesifEvent(HttpExecutionContext ctx, EventModel eventModel) {
        MoesifResource<?> client = ctx.getComponent(ResourceManager.class)
                .getResource("moesif-resource", MoesifResource.class);
        log.info("MoesifResource: {}", client);
        if (client != null) {
